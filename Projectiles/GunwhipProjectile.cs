...
{
	public class GunwhipProjectile : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Timer == swingTime / 2) {
				// Plays a whipcrack sound at the tip of the whip and fires bullets
				...
				if (Main.LocalPlayer == owner)
					foreach (Vector2 point in Projectile.WhipPointsForCollision)
					{
						if (fire-- == 0)
						{
                            Projectile.NewProjectile(owner.GetSource_ItemUse(owner.HeldItem), new Vector2(point.X, point.Y + 24), Projectile.velocity * 3, ModContent.ProjectileType<GunwhipBullet>(), Projectile.damage, Projectile.knockBack);
							fire = 3;
						}
                    }
			}
			...
			if (Utils.GetLerpValue(0.1f, 0.7f, swingProgress, clamped: true) * Utils.GetLerpValue(0.9f, 0.7f, swingProgress, clamped: true) > 0.5f && !Main.rand.NextBool(3)) {
				...
				int dustType = Main.rand.NextBool(3) ? DustID.TintableDustLighted : DustID.Torch;
		...
		private void DrawLine(List<Vector2> list) {
			...
			for (int i = 0; i < list.Count - 1; i++) {
				...
				Color color = Lighting.GetColor(element.ToTileCoordinates(), new Color(67, 47, 33));
    ...
    public class GunwhipPrimeProjectile : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            // This makes the projectile use whip collision detection and allows flasks to be applied to it.
        ...
        public override void AI()
        {
            ...
            if (Timer == swingTime / 2)
            {
                ...
                if (Main.LocalPlayer == owner)
                    foreach (Vector2 point in Projectile.WhipPointsForCollision)
                        Projectile.NewProjectile(owner.GetSource_ItemUse(owner.HeldItem), new Vector2(point.X, point.Y + 24), Projectile.velocity * 2, ModContent.ProjectileType<GunwhipPrimeBullet>(), Projectile.damage, Projectile.knockBack);
            }
